  ...
  <div id="cars-form">
	...
	<table border="0" width="95%" cellspacing="1px" cellpadding="2px">
		...
		<tr>
			...
			<td>will take <input onchange="  if ($(this).val() != \'\' && $(\'input#destination-{{flightNumberNoWS}}\').val() != \'\') { $(\'input#btn-traveltime-{{flightNumberNoWS}}\').removeAttr(\'disabled\'); }  return UtilScraper.get().handleOnChange(\'origin\', \'{{flightNumberNoWS}}\')" id="origin-cartraveltime-{{flightNumberNoWS}}" type="text" size="5" value="30" /> mins</td>
			<td width="250px"><input disabled="disabled" value="Get travel time" onclick="return UtilScraper.get().queryGoogleMap($(\'input#origin-{{flightNumberNoWS}}\').val(), $(\'input#destination-{{flightNumberNoWS}}\').val(), \'span#google-response-{{flightNumberNoWS}}\');" type="button" id="btn-traveltime-{{flightNumberNoWS}}" />&nbsp;&nbsp;<b><span id="google-response-{{flightNumberNoWS}}"></span></b></td>
		</tr>
        <tr>
            <td>From <b id="destination-airport-{{flightNumberNoWS}}">{{destination}} Domestic Airport</b> to</td>
            <td colspan="3"><input id="destination-{{flightNumberNoWS}}" onchange="if ($(this).val() != \'\' && $(\'input#origin-{{flightNumberNoWS}}\').val() != \'\') { $(\'input#btn-traveltime-{{flightNumberNoWS}}\').removeAttr(\'disabled\'); }  return UtilScraper.get().handleOnChange(\'destination\', \'{{flightNumberNoWS}}\')" type="text" size="35" /></td>
        </tr>
		{{/flights}}
		<tr>
