  ...
  <div id="cars-form">
	...
	<table border="0" width="660px" cellspacing="1px" cellpadding="2px">
    	...
    	<tr>
			...
			<td><input onchange="return UtilScraper.get().handleOnChange(this.id, \'{{departureTime}}\', null)" id="pickup-address-to-{{origin}}-airport-{{flightNumberNoWS}}" type="text" size="35" /></td>
			<td>will take <input onchange="return UtilScraper.get().handleOnChange(this.id, \'{{departureTime}}\', this.value)" id="car-traveltime-to-{{origin}}-airport-{{flightNumberNoWS}}" type="text" size="5" /> minutes</td>
		</tr>
        <tr>
            <td>from <b>{{destination}} Domestic Airport</b> to</td>
            <td colspan="2"><input onchange="return UtilScraper.get().handleOnChange(this.id, \'{{departureTime}}\', null)" id="destination-address-from-{{destination}}-airport-{{flightNumberNoWS}}" type="text" size="35" /></td>
        </tr>
		{{/flights}}
		<tr>
